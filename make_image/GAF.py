...
def analyze_dataset_distribution(csv_files):
    ...
    print("\n[推定画像数（ウィンドウサイズ=32の場合）]")

    for label, info in sorted_labels:
        estimated_windows = info["count"] // WINDOW_SIZE
        marker = "🟢" if label == "BENIGN" else "🔴"
...
def extract_windows_3ch(df, feature_cols, scalers, window_size):
    scaled_features = []
    for col in feature_cols:
...
def main():
    ...
    if win_train:
        ...
        n_train = save_images_with_limit(rgb_train, TRAIN_GOOD, prefix="gaf_train",
